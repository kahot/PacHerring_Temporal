...
Fst.random<-read.csv("../Output/Fst/random/Fst_MD2000_random_betweenPWS.SS.csv", row.names = 1)
Fst.random$realFst<-fsts$fst[c(22,20,21)]

fstm<-reshape2::melt(Fst.random[,c("Year","meanFst","realFst","sd")], id.vars=c("Year","sd"))
...
ggplot(fstm, aes(x=Year, y=value, fill=variable))+
    ...
    scale_fill_manual(values=gcols[2:3], labels=c("Randomized", "Estimated Fst"))+
    theme_bw()+theme(legend.title = element_blank())+ggtitle("PWS vs. SS Fst")
ggsave("../Output/Fst/PWS_SS_randomized.vs.realFst.png", width = 4.4, height = 3, dpi=300)
```

![](../Output/Fst/PWS_SS_randomized.vs.realFst.png)
...
fstm<-reshape2::melt(Fst.boot[,c("Year","meanFst","realFst","sd")], id.vars=c("Year","sd"))
fstm$sd[fstm$variable=="realFst"]<-NA
#fstm$Year<-factor(fstm$Year, levels=c("1996","2006","2017"))
...
p2<-ggplot()+
    ...
    scale_color_manual(values=tcols, labels=c("Randomized", "Estimated Fst"))+
...
fstm<-reshape2::melt(Fst.boot[,c("Year","meanFst","realFst","ci_low","ci_high")], id.vars=c("Year","ci_low","ci_high"))
...
p1<-ggplot(fstm, aes(x=factor(Year), y=value, fill=variable))+
    ...
    scale_fill_manual(values=gcols[c(3,2)], labels=c("Randomized", "Estimated Fst"))+ylab(expression("F"[ST] ~" ± 95% CI"))+xlab('')+
    theme_bw()+theme(legend.title = element_blank())+
...
# CIs
bootm<-reshape2::melt(boot, id.vars=c("pop","Year","ci_low","ci_high","P.val") )
...
p4<-ggplot()+
    ...
    scale_color_manual(values=tcols, labels=c("Randomized", expression("Estimated"~ "F"[ST])))+
    theme_bw()+theme(legend.title = element_blank())+
...
# Plot with P-values
fstm<-reshape2::melt(boot[,c("Year","meanFst","realFst","ci_low","ci_high")], id.vars=c("Year","ci_low","ci_high"))
fstm$ci_low[fstm$variable=="realFst"]<-NA
fstm$ci_high[fstm$variable=="realFst"]<-NA
...
p3<-ggplot(bootm, aes(x=Year, y=value,fill=Fst ))+
    ...
    geom_bar(stat="identity", position=position_dodge(width=0.8), width=0.8)+
    geom_errorbar(aes(ymin=ci_low,ymax=ci_high), position=position_dodge(width=0.8), width=.2, size=.2)+
    scale_fill_manual(values=gcols[c(3,2)], labels=c("Randomized", expression("Estimated"~ "F"[ST])))+xlab('')+
